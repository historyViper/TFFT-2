...
### 2.4 Geo_factor: Hamiltonian-Topology Phase Realignment

The geometric factor G is the Malus transmission coefficient for spinor toroid boundary projection. But the deeper explanation is structural: a valid baryon state must simultaneously satisfy two independent constraints:

1. **Continuous Hamiltonian evolution** — the phase must flow forward consistently through the color flux tube winding
2. **Topological closure** — the toroid must close on itself after the full spinor 720° rotation

These two constraints are not automatically compatible. For most projection angles, satisfying one forces a discontinuous "snap" in the other — a discrete rotational transition that manifests as a topology change rather than a mass correction. The geo_factor values are the **specific projection angles where both constraints can be satisfied simultaneously without a snap transition**.

This is why the geo_factor takes discrete values rather than a continuous range:

| Generation | S2[gen] = sin²(n·π/15) | Value | Quarks | Physical meaning |
|------------|------------------------|-------|--------|-----------------|
| gen1 (n=4) | sin²(48°) | 0.552264 | up, down | Both constraints close at 48° for gen1 winding |
| gen2 (n=7) | sin²(84°) | 0.989074 | strange, charm | Near-maximal alignment — charm/strange high coherence |
| gen3 (n=2) | sin²(24°) | 0.165435 | bottom, top | Low projection — heavy quark high inertia |
| boundary (n=1) | sin²(12°) | 0.043227 | GEO_B (floor) | Minimum valid coupling — boundary quantum |

A baryon with a projection angle between these values would require a snap transition to realign — which is exactly what the minimum topology rule enforces. The topology assignments (T1/T2/T3, S1/S2) are the discrete realignment states the system snaps to when the Hamiltonian and topology constraints cannot be simultaneously satisfied at the current projection angle.

**In Malus's Law terms:** the toroid phase rotates and the 3D observable boundary is a fixed polarizing filter. When phases align at one of the four valid geo_factor angles, maximum energy transmits. Misalignment at any other angle produces tension that forces a topology transition rather than a smooth correction — the system snaps rather than bends.
...
The minimum topology rule is a direct consequence of the Hamiltonian-topology constraint described in Section 2.4. When quark generation mixing increases, the mismatch between the Hamiltonian phase evolution and the toroid closure angle grows beyond the valid geo_factor positions. The system must snap to the next discrete realignment state — a higher topology cover. S1/T1 is the trivial state (no snap required); T2 is the first helicity-flip realignment; T3 is the full topology-flip realignment where both phase and topology rotations complete together.

| Condition | Min. topology | Physical reason |
|-----------|--------------|-----------------|
| Pure same-generation quarks | S1/T1 allowed | Hamiltonian and topology close together naturally |
| Direct nucleon spin excitation (Delta+/0/−) | S1/T1 allowed | Same winding as parent — no realignment needed |
| No nucleon parent (Delta++/Delta−) | T2 minimum | HE21 asymmetric figure-8 — helicity flip required |
| Any heavy quark (c/b/t) | T2 minimum | Cross-generation coupling forces first snap |
| Three different generations (d,s,b) | T3 preferred | Three-axis constraint needs double rotation to close |
| Three strange quarks | S2 minimum | Accumulated inertial mismatch forces sheet crossing |

S1/T1 is **forbidden** for topologically complex configurations because the Hamiltonian and topology constraints cannot both be satisfied at the trivial projection — the system must snap to at least T2 to realign. This is why the ablation study shows topology assignments are load-bearing (+2.63% when forced to S1/T1): removing topology is not removing a fitting choice, it is removing the physical realignment mechanism.
